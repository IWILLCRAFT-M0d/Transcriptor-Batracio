...
// Función de agregar lineas y aumentar el tamaño del textarea
let agre0 = (e) => {
...
<button class="botonagregar" onclick="agre0(this)">Agregar</button><label></label><button class="botoneliminar" onclick="elim(this)">Eliminar</button><br>
<button class="mezclar" id="" onclick="mezclarA(this)" disabled style="left: 27.5%;">Mezclar diálogos</button><button class="mezclar" id="" onclick="mezclarB(this)" disabled style="visibility: hidden;">Mezclar diálogos Español</button>`;
            break
    };
    din.insertBefore(div, din.children[idsel2 + 1]);
    recalcularIds()
    var resultadoAltura = document.getElementById("resultado").style.height;
    var resultadoAlturaNumero = resultadoAltura.match(/\d+/);
    let resultadoAlturaSuma = parseInt(resultadoAlturaNumero) + 50;
...
let elim = (e) => {
    ...
    let idsel1 = idsel0.children[0].id.match(/\d+/);
    let idsel2 = parseInt(idsel1);
    if (dinTotal > 0 && idsel2 > 0) {
        let divM = e.parentNode;
        din.removeChild(divM);
        recalcularIds();
        var resultadoAltura = document.getElementById("resultado").style.height;
        var resultadoAlturaNumero = resultadoAltura.match(/\d+/);
        let resultadoAlturaResta = parseInt(resultadoAlturaNumero) - 50;
        document.getElementById("resultado").style.height = resultadoAlturaResta + "px";
    }
...
din.addEventListener("keyup", e => {
    ...
    switch (event.target.id) {
        ...
        case 'ding' + limite:
            if (document.getElementById('ding' + digitosRun).value.length > 0 && document.getElementById('ding' + digitos).value.length > 0) {
                document.getElementById('mezclarA' + digitosRun).removeAttribute('disabled', "")                
            }      
            else {
                document.getElementById('mezclarA' + digitosRun).setAttribute('disabled', "")
            }
            break

        case 'ding' + digitos:
            if (document.getElementById('ding' + digitos).value.length > 0 && document.getElementById('ding' + digitosFix).value.length > 0) {
                document.getElementById('mezclarA' + digitos).removeAttribute('disabled', "")
            } else if (document.getElementById('ding' + digitos).value.length > 0 && document.getElementById('ding' + digitosRun).value.length > 0) {
                document.getElementById('mezclarA' + digitosRun).removeAttribute('disabled', "")
            } else {
                document.getElementById('mezclarA' + digitosRun).setAttribute('disabled', "")
                document.getElementById('mezclarA' + digitos).setAttribute('disabled', "")
            }
        break       
        ...
        case 'desp' + limite:
            if (document.getElementById('desp' + digitosRun).value.length > 0 && document.getElementById('desp' + digitos).value.length > 0) {
                document.getElementById('mezclarB' + digitosRun).removeAttribute('disabled', "")                
            }      
            else {
                document.getElementById('mezclarB' + digitosRun).setAttribute('disabled', "")
            }
            break

        case 'desp' + digitos:
            if (document.getElementById('desp' + digitos).value.length > 0 && document.getElementById('desp' + digitosFix).value.length > 0) {
                document.getElementById('mezclarB' + digitos).removeAttribute('disabled', "")
            } else if (document.getElementById('desp' + digitos).value.length > 0 && document.getElementById('desp' + digitosRun).value.length > 0) {
                document.getElementById('mezclarB' + digitosRun).removeAttribute('disabled', "")
            } else {
                document.getElementById('mezclarB' + digitosRun).setAttribute('disabled', "")
                document.getElementById('mezclarB' + digitos).setAttribute('disabled', "")
            }
        break
    }
    ...
    switch (cambioIdioma.checked) {
        ...
        switch (event.target.id) {
            ...
            case "desp" + digitos:
                if ((document.getElementById('per' + (digitos)).value.length === 0 && document.getElementById('ding' + (digitos)).value.length === 0 && document.getElementById('desp' + (digitos)).value.length === 0)) {
                    document.getElementById("per" + digitos).focus();
                }

                else if ((document.getElementById('per' + (digitos)).value.length === 0 && document.getElementById('ding' + (digitos)).value.length > 0 && document.getElementById('desp' + (digitos)).value.length > 0)) {
                    document.getElementById("per" + digitos).focus();
                }

                else if ("desp" + digitos === "desp" + (din.children.length - 1)) {
                    agre0();
                    document.getElementById("per" + (din.children.length - 1)).focus();
                }

                else if ("desp" + digitos < "desp" + (limite)) {
        ...
        case true:
            switch (event.target.id) {
                ...
                case "ding" + digitos:
                    if ((document.getElementById('per' + (digitos)).value.length === 0 && document.getElementById('ding' + (digitos)).value.length === 0 && document.getElementById('desp' + (digitos)).value.length === 1)) {
                        document.getElementById("per" + digitos).focus();
                    }
    
                    else if ((document.getElementById('per' + (digitos)).value.length === 0 && document.getElementById('ding' + (digitos)).value.length > 0 && document.getElementById('desp' + (digitos)).value.length > 1)) {
                        document.getElementById("per" + digitos).focus();
                    }
    
                    else if ("ding" + digitos === "ding" + (din.children.length - 1)) {
                        agre0();
                        document.getElementById("per" + (din.children.length - 1)).focus();
                    }
    
                    else if ("ding" + digitos < "ding" + (limite)) {
                        document.getElementById("per" + digitosFix).focus();
                    }
                    break
            }
